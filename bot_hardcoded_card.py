...
# Click orqali to'lov qilinadigan HUMO karta raqami.
# Railway Variables ichida CARD_NUMBER nomi bilan saqlang.
CARD_NUMBER = "BU_YERGA_KARTA_RAQAMINGIZNI_YOZING".replace(" ", "").strip()
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not CARD_NUMBER:
        await update.message.reply_text(
            "⚠️ Click to‘lov kartasi hali sozlanmagan. Administrator CARD_NUMBER ni Railway Variables ichiga qo‘shishi kerak.",
            reply_markup=MENU,
        )
        return
    ...
    click_url = (
        ...
        f"&amount={DAILY_PACK_PRICE_UZS}"
    )

    keyboard = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                text=f"💳 Click orqali to‘lash ({DAILY_PACK_PRICE_UZS:,} so‘m)".replace(",", " "),
                url=click_url,
            )
        ]
    ])
    ...
    text = (
        ...
        f"{DAILY_PACK_QUESTIONS} ta qo‘shimcha savol — {DAILY_PACK_PRICE_UZS:,} so‘m.\n"
        f"Paket faqat bugun amal qiladi.\n\n"
        f"Pastdagi tugmani bosing va Click orqali to‘lov qiling.\n\n"
        f"To‘lovdan keyin administrator tasdiqlaydi va "
        f"bugungi {DAILY_PACK_QUESTIONS} ta savol paketingiz faollashadi.\n\n"
        f"Sizning Telegram ID: {update.effective_user.id}"
    ).replace(",", " ")
